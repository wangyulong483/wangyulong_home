...
## 需求
优化全站交互流畅度和访问性能，在保持现有页面视觉设计、首页视频转场、紫色粒子及人物主题的前提下，解决滚动响应滞后、弱设备持续渲染压力以及神社页首屏加载卡顿。

- 首页转场需紧跟滚动手势，减少“滚动后动画才追上来”的延迟感
- 降低全屏视频转场和紫色粒子的持续 GPU/CPU 开销
- 移除可能在中国大陆阻塞页面显示的 Google Fonts 第三方请求
- 压缩神社页 12-19 MB 的超大 PNG，并降低首屏布局偏移
- 保持桌面端、390px 移动端的现有美观效果和人物视觉重点
- 清理页面切换后遗留的事件监听与非必要首屏网络请求

## 方案

### 1. 首页滚动转场提速

- 将 `HeroTransition.vue` 的 GSAP `scrub` 跟随时间从 `0.65` 缩短到约 `0.2`，保留柔和惯性但显著提升跟手性
- 移除全屏视频逐帧 `blur()` 动画，只使用合成层友好的 `opacity` 与 `transform` 完成淡出
- 避免 `onUpdate` 每帧重复写入相同的 `visibility` 状态与重复派发无变化的阶段状态
- 转场结束后释放长期 `will-change`，降低显存占用；兼容 `prefers-reduced-motion`

### 2. 消除字体网络阻塞

- 删除 `index.html` 中 Google Fonts 的跨域预连接和在线样式请求
- 改用本机现代字体栈：正文优先 `Segoe UI Variable`、`PingFang SC`、`Microsoft YaHei`，等宽内容优先 `Cascadia Code`、`Consolas`
- 保持当前字号、字重与版式层级，避免字体切换造成明显视觉变化和布局跳动

### 3. 紫色粒子性能优化

- 将粒子两两全量距离检测改为空间网格邻域查询，减少每帧无效计算
- 桌面粒子数调整为约 84，移动端约 32，并根据设备性能限制 Canvas DPR 或刷新频率
- 页面不可见时暂停动画，恢复后继续；保留紫色、鼠标吸附、近距离连线与移动端视觉效果
- 控制 Canvas 尺寸更新和粒子重建频率，避免 resize 期间频繁分配

### 4. 神社页图片与首屏加载优化

- 将 12-19 MB、最高 5000x5000 的 PNG 转为适配页面实际展示尺寸的 WebP；透明人物图保留 Alpha 通道
- 人物图输出约 1000-1200px，生日轮播图约 1400-1600px，并更新资源索引与兜底路径
- 为图片补充稳定宽高、`decoding="async"` 与合理的 `fetchpriority`，降低 CLS
- 轮播仅加载当前图及相邻图，避免浏览器提前请求全部大图
- 将非首屏 B 站数据请求延后到对应内容临近视口或用户交互时触发

### 5. 生命周期清理与验证

- 修复 `About.vue` 全局键盘监听在组件卸载后未清理的问题，防止反复导航累积监听器
- 执行生产构建，并对比优化前后的资源体积、首页滚动帧时间、神社页 LCP/CLS
- 使用桌面端与 390px 移动端检查首页转场、粒子、人物主题、图片清晰度及布局稳定性
- 完成后提交 Git；本轮不自动部署，除非用户再次明确要求部署
...
移除亮暗色主题切换，全站只保留亮色主题；首页在视频转场结束后显示用户提供的 canvas-nest 风格蓝色交互粒子效果。

- 删除侧边栏中的主题切换按钮，不再读写 `localStorage.theme`
- 页面首次加载即为亮色，不出现暗色首帧或主题闪烁
- 粒子使用 `45,118,200` 蓝色，保留随机漂移、近距离连线、鼠标吸附和指针离开复位
- 开场全屏视频阶段仍只显示视频；视频淡出、首页内容出现后再显示粒子
- 移动端保留粒子效果，但降低密度以控制耗电与帧率

## 方案

### 1. 固定亮色主题

- `frontend/index.html` 固定 `data-theme="light"`、`color-scheme: light` 和亮色浏览器主题色
- 删除读取 `localStorage` 的防闪烁脚本，因为主题不再可变
- 将 `App.vue` 的 `:root` 设计变量直接改为当前亮色变量，移除重复的 `[data-theme="light"]` 变量覆盖
- 保留部分组件现有的 `[data-theme="light"]` 定向样式，使其他页面继续使用已经设计好的亮色表现

### 2. 删除主题切换 UI

- 从 `Sidebar.vue` 删除主题按钮、太阳/月亮图标与相关 CSS
- 删除 `isLight`、`toggleTheme`、`syncTheme` 和主题存储逻辑
- 保留白色桌面侧栏和移动端人物身份抽屉，其余导航行为不变

### 3. 重构交互粒子

以用户提供的 IIFE 算法为行为基准，在现有 Vue 组件中实现，不直接向 `body` 注入脚本：

- 固定 RGB 为 `45,118,200`，桌面默认 `count=100`、`opacity=0.8`
- 移动端使用约 42 个粒子、`opacity=0.45`，仍保留粒子与连线
- 保留平方距离阈值：粒子间 `max=6000`、鼠标点 `max=20000`
- 使用 `requestAnimationFrame` 时间差校正速度，避免 120Hz 屏幕移动过快
- 按 `devicePixelRatio` 缩放 canvas，使高分屏线条清晰，并限制 DPR 防止像素开销过高
- 监听 `count` 变化并重建粒子，修复移动端初始仍生成 100 个粒子的问题
- 页面隐藏时暂停，恢复时继续；组件卸载时清理 RAF、定时器及所有事件监听
- `prefers-reduced-motion` 下渲染静态粒子帧，不持续动画

### 4. 首页显示时机

- 延续 `heroProgress > 0.6` 后显示粒子的现有规则，保证视频首屏纯净
- 非首页继续保留粒子背景，统一全站亮色视觉
- canvas 保持 `pointer-events: none`，不遮挡链接、菜单和滚动

### 涉及文件

| 文件 | 改动 |
|------|------|
| `frontend/index.html` | 固定亮色并删除主题初始化脚本 |
| `frontend/src/App.vue` | 亮色变量常态化，设置自适应粒子参数 |
| `frontend/src/components/Sidebar.vue` | 删除主题切换 UI 和逻辑 |
| `frontend/src/components/ParticleBackground.vue` | 按提供算法重构粒子渲染与生命周期 |
| `todo.md` | 记录本次需求与方案 |

### 验证

1. 清空/预设旧 `localStorage.theme=dark` 后刷新，确认始终亮色且无暗色闪烁
2. 检查首页 scroll=0 时只有视频，转场后蓝色粒子出现
3. 验证鼠标附近连线与吸附、窗口缩放、标签页隐藏/恢复
4. 在 390×844 移动端确认粒子存在、页面无横向溢出且滚动顺畅
5. 执行完整 `npm run build`，完成后提交 Git

===============================================================

## 需求

调整首页及全站粒子颜色为紫色；“关于”页面中已有 BV 号、可以播放的视频卡片必须显示对应封面，方便用户快速区分视频内容。

- 粒子从蓝色改为与站点强调色一致的紫色
- 已发布视频优先显示真实 B 站封面
- 区分可播放视频的封面加载状态与尚未上传的视频占位状态
- 封面加载失败时保持稳定占位，不影响卡片点击播放

## 方案

### 1. 紫色粒子

- 将 `ParticleBackground.vue` 的默认 RGB 从 `45,118,200` 调整为品牌紫 `108,92,231`
- 保持当前粒子数量、透明度、连线、鼠标吸附和移动端性能策略不变

### 2. 关于页视频封面

- 将三个已发布视频的真实封面保存到 `public/video-covers/` 并直接绑定，避免运行时接口或 CDN 不稳定
- 保留现有根据 BV 号调用 B 站 view API 获取封面的逻辑，供后续新增视频自动补充封面
- 封面图片增加 `referrerpolicy="no-referrer"`，避免 B 站 CDN 因来源校验拒绝图片
- 为封面增加加载失败处理；失败后显示“视频已发布”占位，避免破图
- 有 BV 号但仍在加载时显示“封面加载中”，无 BV 号时继续显示“待上传”
- 图片继续使用懒加载，并维持固定宽高比，避免布局跳动
- 限定封面图片选择器，防止占位图标被拉伸到整张卡片
- 修复非首页桌面侧栏样式覆盖移动端抽屉位移的问题，确保移动端默认收起侧栏

### 涉及文件

| 文件 | 改动 |
|------|------|
| `frontend/src/components/ParticleBackground.vue` | 粒子默认颜色改为紫色 |
| `frontend/src/views/About.vue` | 修复 B 站封面显示及加载失败状态 |
| `frontend/src/components/Sidebar.vue` | 修复非首页移动端侧栏默认展开 |
| `frontend/public/video-covers/` | 三个可播放视频的本地封面资源 |
| `todo.md` | 记录本次需求与方案 |

### 验证

1. 首页转场完成后确认粒子为紫色，非首页粒子颜色一致
2. “关于”页三个带 BV 号的视频卡片显示不同封面
3. 模拟封面请求失败时显示稳定占位，不出现破图
4. 桌面端与移动端检查卡片比例、文字和点击播放
5. 执行完整 `npm run build` 并提交 Git

===============================================================

## 需求

美化页面最右侧的浏览器纵向滚动条，使其与亮色页面和紫色品牌视觉协调，弱化默认滚动条的生硬直线感，同时保持易用性。

- 页面滚动条更加轻盈、圆润，不抢正文视觉
- 保留清晰的滚动位置提示和鼠标拖动能力
- 悬停与拖动时提供适度反馈
- 不影响移动端系统滚动体验和现有内部滚动区域

## 方案

### 1. 全局滚动条视觉

- 在 `App.vue` 全局样式中集中定义滚动条颜色变量
- 桌面端滚动轨道保持透明，消除最右侧整条灰线
- 滑块使用低透明度品牌紫、胶囊圆角和透明描边，形成与页面背景分离的细悬浮效果
- 默认状态保持克制，悬停时提高紫色对比度，拖动时进一步加深
- 隐藏旧式上下箭头按钮，避免多余视觉元素

### 2. 浏览器与设备兼容

- Edge、Chrome、Safari 使用 `::-webkit-scrollbar` 系列实现圆角、描边和交互状态
- Firefox 使用标准 `scrollbar-color` 与 `scrollbar-width`
- 仅在支持鼠标精确指针的桌面环境启用细化样式；触屏设备继续使用系统原生悬浮滚动条
- 不修改 `ChatTab`、横向标签栏等组件自己的滚动策略

### 涉及文件

| 文件 | 改动 |
|------|------|
| `frontend/src/App.vue` | 新增全局桌面滚动条样式与兼容规则 |
| `todo.md` | 记录本次需求与方案 |

### 验证

1. 桌面 Edge 检查长页面滚动条默认、悬停和拖动状态
2. 首页视频转场前后确认右侧没有突兀轨道或布局跳动
3. “关于”、热点和神社长页面检查滚动位置提示清晰
4. 390px 移动端确认不强制显示桌面滚动条且无横向溢出
5. 执行完整 `npm run build` 并提交 Git
